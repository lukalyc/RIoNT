...
use riont_store::pose::PoseReading;
use riont_store::pose::PoseSource;
...
/// The subset of app settings the field resolver needs — deliberately
/// decoupled from any one application's config types.
pub struct FieldSettingsRef<'a> {
    /// External JSON (PathPlanner format); overrides built-ins while set.
    pub walls_file: Option<&'a str>,
    /// Built-in map name (BUILTIN_MAPS entry).
    pub map: &'a str,
}

/// Resolve the active field map from settings. Returns the map plus an
/// optional warning (walls_file failed to read/parse) for the caller to
/// toast — the built-in fallback always succeeds. Priority:
///
/// 1. `walls_file` (explicit external JSON)
...
pub fn resolve(settings: FieldSettingsRef) -> (FieldMap, Option<String>) {
    let walls_file = settings.walls_file.unwrap_or("");
    if !walls_file.is_empty() {
        let path = walls_file;
        return match std::fs::read_to_string(path) {
            Ok(text) => match FieldMap::from_json(&text, path) {
                Ok(m) => (m, None),
                Err(e) => (builtin_map(settings.map), Some(e)),
            },
            Err(e) => (
                builtin_map(settings.map),
                Some(format!("walls_file {}: {}", path, e)),
            ),
        };
    }
    if let Ok(text) = std::fs::read_to_string(format!("fields/{}.json", settings.map)) {
        if let Ok(m) = FieldMap::from_json(&text, settings.map) {
            return (m, None);
        }
    }
    (builtin_map(settings.map), None)
}
...
/// resolve()'s job.
fn builtin_map(map: &str) -> FieldMap {
    // Unknown names fall back to the engine default (2026-rebuilt); the
    // default is built in, so this cannot fail.
    FieldMap::builtin(map)
        .or_else(|| FieldMap::builtin("2026-rebuilt"))
...
pub fn forced_reading(value: &NtValue) -> Option<PoseReading> {
    match value {
        NtValue::DoubleArray(v) if v.len() >= 6 => Some(PoseReading {
            x: v[0],
            y: v[1],
            radians: v[5].to_radians(),
            source: PoseSource::Limelight,
        }),
        NtValue::Pose2d { x, y, radians } => Some(PoseReading {
            x: *x,
            y: *y,
            radians: *radians,
...
mod tests {
    ...
    fn resolve_falls_back_to_builtin_on_bad_file() {
        let settings = FieldSettingsRef {
            walls_file: Some("definitely/missing.json"),
            map: "2026-rebuilt",
        };
        let (m, warn) = resolve(settings);
        assert!(warn.is_some());
        assert!(!m.walls.is_empty());
    }
